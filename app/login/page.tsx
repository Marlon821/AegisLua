      ...
      <section className="relative grid w-full max-w-md gap-10">
        ...
        <form className="glass-card rounded-2xl p-8" onSubmit={submit}>
          <div className="mb-7 grid grid-cols-2 rounded-xl border border-white/10 bg-black/40 p-1">
            <button className={`rounded-lg px-4 py-2 text-sm font-black transition ${authMode === "login" ? "bg-lime-300 text-[#101409]" : "text-zinc-500 hover:bg-white/10 hover:text-white"}`} onClick={() => setAuthMode("login")} type="button">
              Login
            </button>
            <button className={`rounded-lg px-4 py-2 text-sm font-black transition ${authMode === "signup" ? "bg-lime-300 text-[#101409]" : "text-zinc-500 hover:bg-white/10 hover:text-white"}`} onClick={() => setAuthMode("signup")} type="button">
              Sign up
            </button>
          </div>
          ...
          {error ? <div className="mt-5 rounded-xl border border-amber-300/30 bg-amber-300/10 px-4 py-3 text-sm text-amber-100">{error}</div> : null}
          ...
          {resetUrl ? (
            <a className="mt-3 block break-all rounded-xl border border-lime-300/30 bg-lime-300/10 px-4 py-3 text-sm text-lime-100 transition hover:bg-lime-300/15" href={resetUrl}>
          ...
          <div className="mt-6 grid gap-4">
            {authMode === "signup" ? (
              <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">
                Name
                <input className="rounded-xl border border-white/10 bg-black/45 px-4 py-3 text-white outline-none transition focus:border-lime-300" value={name} onChange={(event) => setName(event.target.value)} />
              </label>
            ) : null}
            {authMode !== "new-password" ? (
              <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">
                Email
                <input className="rounded-xl border border-white/10 bg-black/45 px-4 py-3 text-white outline-none transition focus:border-lime-300" value={email} onChange={(event) => setEmail(event.target.value)} type="email" />
              </label>
            ) : null}
            {authMode !== "reset" ? (
              <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">
                ...
                <span className="relative">
                  <input className="w-full rounded-xl border border-white/10 bg-black/45 px-4 py-3 pr-11 text-white outline-none transition focus:border-lime-300" value={password} onChange={(event) => setPassword(event.target.value)} type="password" />
          ...
          <button className="mt-6 w-full rounded-xl bg-lime-300 px-5 py-3 font-black text-[#101409] shadow-lg shadow-lime-950/30 transition hover:-translate-y-0.5 hover:bg-lime-200 disabled:opacity-60" disabled={loading} type="submit">
            {loading ? "Working..." : submitLabel}
          </button>

          {authMode === "login" ? (
            <button className="mt-4 w-full text-center text-sm font-semibold text-zinc-500 transition hover:text-lime-300" onClick={() => setAuthMode("reset")} type="button">
              Forgot password?
            </button>
          ) : null}
          ...
          <p className="mt-8 text-center text-sm text-zinc-500">
            ...
            <button className="font-semibold text-lime-300 hover:text-lime-200" onClick={() => setAuthMode(authMode === "login" ? "signup" : "login")} type="button">
